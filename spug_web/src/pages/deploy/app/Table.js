    ...
    function expandedRowRender(record) {
        return (
            <Table
                ...
                pagination={false}>
                {/*修改Table.Column中render函数，添加对extend='3'的处理*/}
                <Table.Column width={80} title="模式" dataIndex="extend" render={value =>
                    value === '1' ? <OrderedListOutlined style={{fontSize: 20, color: '#1890ff'}}/> :
                        value === '2' ? <BuildOutlined style={{fontSize: 20, color: '#1890ff'}}/> :
                            <BuildOutlined style={{fontSize: 20, color: '#1890ff'}}/>
                }/>
                <Table.Column title="发布环境" dataIndex="env_id"
                ...
                {hasPermission('deploy.app.config|deploy.app.edit') && (
                    <Table.Column title="操作" render={info => (
                        <Action>
                            ...
                            <Action.Button danger auth="deploy.app.edit"
                                           onClick={() => handleDeployDelete(info)}>删除</Action.Button>
                        </Action>
                    )}/>
                )}
            </Table>
            ...
            {hasPermission('deploy.app.edit|deploy.app.del') && (
                <Table.Column width={260} title="操作" render={info => (
                    <Action>
                        ...
                        <Action.Button auth="deploy.app.edit"
                                       onClick={e => handleClone(e, info.id)}>克隆发布</Action.Button>
                        <Action.Button auth="deploy.app.edit"
                                       onClick={e => store.showForm(e, info)}>编辑</Action.Button>
                        <Action.Button danger auth="deploy.app.del"
                                       onClick={e => handleDelete(e, info)}>删除</Action.Button>
                    </Action>
                )}/>
            )}
        </TableCard>
